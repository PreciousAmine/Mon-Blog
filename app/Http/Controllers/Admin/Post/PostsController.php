...
class PostsController extends Controller
{
	...
	public function store(CreatePostRequest $request)
	{
		...
		$data['video'] = $request->file('image')->store('images/blogs', 'public');

		Posts::create($data);

		return to_route('article.index')->with('success', 'Poem has been created.');
	}
	...
	public function update(UpdatePostRequest $request, string $id)
	{
		...
		return to_route('article.index')->with('info', 'Poem has been updated.');
	}
	...
	public function destroy(string $id)
	{
		...
		if ($post->active === 1) {
			return to_route('article.index')->with('warning', 'This poem already published. Set to unpublished first to delete poem.');
		}
		...
		return to_route('article.index')->with('danger', 'Poem has been deleted.');
	}
}
